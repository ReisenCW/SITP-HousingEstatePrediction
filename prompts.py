...
PROMPTS = {
    ...
    幅度可以是具体值或范围（如"1%-3%"、"0.5%以内"），对于预测时间范围小于6个月的, 趋势可以为上升/下降/持平, 对于大于6个月的, 还可以分段预测, 如先上升后下降。
    信息：{info}
    ...
    # 获取实际趋势
    "get_actual_trend": """
    请联网查询并分析{region}在{time_range}的实际房价均价变化趋势及幅度。幅度可以是具体值或范围（如"1%-3%"、"0.5%以内"），对于预测时间范围小于6个月的, 趋势可以为上升/下降/持平, 对于大于6个月的, 还可以分段预测, 如先上升后下降。
